...
namespace LevelStrategy.BL
{
    public class Strategy
    {
        ...
        public static void FindSignal(Bars bars, EventHandler<SignalData> eventHandler)
        {
            ...
            {
                bars.listSignal.Add(new SignalData(bars.NumberGrid, "Short - Повторяющийся уровень", bars.Time[bsy], bars.Time[(bar - 1)], bars.Time[bar], bars.High[bsy], bars.High[bsy] * 0.9996, ((bars.High[bsy] * 0.9996) * 0.996), DateTime.Now, bars.Name));
            ...
            {
                bars.listSignal.Add(new SignalData(bars.NumberGrid, "Short - Зеркальный уровень", bars.Time[bsy], bars.Time[(bar - 1)], bars.Time[bar], bars.Low[bsy], bars.Low[bsy] * 0.9996, ((bars.Low[bsy] * 0.9996) * 0.996), DateTime.Now, bars.Name));
                eventHandler(new object(), bars.listSignal.Last());
            }
            if (AirLevel(bar, bars, out bsy) == "Short")//&& DefenitionAreaNearLevel(bar) == "Nothing" && shortTrade)
            {
                bars.listSignal.Add(new SignalData(bars.NumberGrid, "Short - Воздушный уровень", bars.Time[bsy], bars.Time[(bar - 1)], bars.Time[bar], bars.Low[bsy], bars.Low[bsy] * 0.9996, ((bars.Low[bsy] * 0.9996) * 0.996), DateTime.Now, bars.Name));
            ...
            {
                bars.listSignal.Add(new SignalData(bars.NumberGrid, "Long - Повторяющийся уровень", bars.Time[bsy], bars.Time[(bar - 1)], bars.Time[bar], bars.Low[bsy], bars.Low[bsy] * 1.0004, ((bars.Low[bsy] * 1.0004) * 1.004), DateTime.Now, bars.Name));
                eventHandler(new object(), bars.listSignal.Last());
            }
            ...
            {
                bars.listSignal.Add(new SignalData(bars.NumberGrid, "Long - Зеркальный уровень", bars.Time[bsy], bars.Time[(bar - 1)], bars.Time[bar], bars.High[bsy], bars.High[bsy] * 1.0004, ((bars.High[bsy] * 1.0004) * 1.004), DateTime.Now, bars.Name));
                eventHandler(new object(), bars.listSignal.Last());
            }
            if (AirLevel(bar, bars, out bsy) == "Long")// && DefenitionAreaNearLevel(bar) == "Nothing" && longTrade)
            {
                bars.listSignal.Add(new SignalData(bars.NumberGrid, "Long - Воздушный уровень", bars.Time[bsy], bars.Time[(bar - 1)], bars.Time[bar], bars.High[bsy], bars.High[bsy] * 1.0004, ((bars.High[bsy] * 1.0004) * 1.004), DateTime.Now, bars.Name));
                ChangeColorConsole(true);
                eventHandler(new object(), bars.listSignal.Last());
                ChangeColorConsole(false);
            }
